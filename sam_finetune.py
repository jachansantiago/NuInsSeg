...
def get_sam_model():
    sam = sam_model_registry[args.arch](args,checkpoint=os.path.join('weights', args.sam_ckpt),num_classes=args.num_cls)
...
def train_model(sam, trainloader,valloader,dir_checkpoint,epochs):
    ...
    for epoch in pbar:
        ...
        for i,data in enumerate(tqdm(trainloader)):
            # print("Image shape:",data['image'].shape, data['image'].dtype)
            # print("Mask shape:",data['mask'].shape, data['mask'].dtype)
            imgs = data['image'].cuda()
            msks = torchvision.transforms.Resize((args.out_size,args.out_size))(data['mask'])
            msks = msks.cuda()
            # print("Mask shape after:",msks.shape)
